...
class CACCrawler(BaseCrawler):
    ...
    def __init__(self, source_name: str = None, source_url: str = None):
        ...
        self.base_url = "http://www.cac.gov.cn"
        self.headers = {
    ...
    def fetch(self) -> List[Dict]:
        ...
        try:
            # 爬取政策法规
            url = f"{self.base_url}/gzzt/ztzl/zcfg/"
            ...
            # 查找新闻列表
            news_items = soup.select('.list_txt li')[:10]
            ...
            self.logger.info(f"网信办爬取成功，获取 {len(articles)} 篇文章")
            
        except Exception as e:
            self.logger.error(f"网信办爬取失败: {e}")
